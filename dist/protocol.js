...
class Protocol {
     ...
     */
    constructor(config = {}) {
        if (config.CHAR_MAIN_SEP)
            this.config.CHAR_MAIN_SEP = config.CHAR_MAIN_SEP;
        if (config.CHAR_ESCAPE)
            this.config.CHAR_ESCAPE = config.CHAR_ESCAPE;
    }
    decodeRaw(s = '') {
        let str = '';
        let cur = 0;
        ...
        while (sp !== -1) {
            if (s[sp + 1] !== this.config.CHAR_MAIN_SEP)
                str += s.slice(cur, sp + 1);
            else
                str += `${s.slice(cur, sp)}${this.config.CHAR_MAIN_SEP}`;
            cur = sp + 2;
    ...
    decode(packet) {
        ...
        while (sp !== -1) {
            ...
            if (packet.payload[sp - 1] !== this.config.CHAR_ESCAPE
